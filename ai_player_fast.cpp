...
int
ai_player_fast::get_steps_to_win(TicTacToeBoard current_board, int player) {
    ...
    while(cur_board_ind < generated_boards.size()){
        ...
        for( std::tuple<int,int> coord : free_in_cur_board){
            ...
            if (cur_player == X_PLAYER) {
                new_board.set_cross(std::get<0>(coord), std::get<1>(coord));
            } else {
                new_board.set_zero(std::get<0>(coord), std::get<1>(coord));
            }
...
std::tuple<int, int> ai_player_fast::get_next_step(std::tuple<int, TicTacToeBoard> state) {
    ...
    for (std::tuple<int, int> coordinates : free_coords) {
        TicTacToeBoard temp_board(board);
        temp_board.set_zero(std::get<0>(coordinates), std::get<1>(coordinates));
        int current_min_steps_to_win = get_steps_to_win(temp_board, player);//Is it copy? Should be copy
